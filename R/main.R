...
#' @param python.dir the path to Python, default is "python3".
#'
#' @examples
#' ## input files
...
#'   target.bed, reference, SNP.database, samtools.dir, sample.id
#' )
#' @export
parameter_recommend <- function(plasma.unmerged, normal,
                            plasma.merged.extendedFrags, plasma.merge.notCombined,
                            target.bed, reference, SNP.database, samtools.dir,
                            sample.id, roughly_estimated_tf = FALSE, python.dir = "python3") {
  ...
  cat(paste0("average = ", signif(as.numeric(depth_results[1]), 6)))
  cat(paste0(", median = ", signif(as.numeric(depth_results[2]), 6)))
  ...
  if (roughly_estimated_tf) {
    ...
    cat("For a more accurate estimation, please run variant_calling().", "\n")
  }
  ...
  if (p95 >= 500) {
    cat(paste0("at 0.5% VAF: MIN_HOLD_SUPPORT_COUNT = ",
               round(0.005*p95, digits = 0)+6, ", MIN_PASS_SUPPORT_COUNT = ",
               round(0.005*p95, digits = 0), ";"), "\n")
  }
  cat(paste0("at 1% VAF: MIN_HOLD_SUPPORT_COUNT = ",
             round(0.01*p95, digits = 0)+6, ", MIN_PASS_SUPPORT_COUNT = ",
             round(0.01*p95, digits = 0), ";"), "\n")
  cat(paste0("at 5% VAF: MIN_HOLD_SUPPORT_COUNT = ",
             round(0.05*p95, digits = 0)+6, ", MIN_PASS_SUPPORT_COUNT = ",
             round(0.05*p95, digits = 0)), "\n")
  cat("Note: decreasing the parameters (i.e. MIN_HOLD_SUPPORT_COUNT and MIN_PASS_SUPPORT_COUNT) \ncan lower the detection limit, but may also lower the variant quality.")
...
variant_calling <- function(plasma.unmerged, normal,
                            ...
                            java.dir = "java", python.dir = "python3") {
